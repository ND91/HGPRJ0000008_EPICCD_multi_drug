#!/usr/bin/env R
# Perform DMP analyses comparing CRP at T1.
...
design_mat_t1 <- model.matrix(~CRP, data = sample_metadata_t1)
...
dmps_T1CRP <- data.frame(topTable(mvals_ebayes, coef = "CRP", number = "Inf", adjust.method = "BH", sort.by = "p")) %>%
...
write.csv(dmps_T1CRP, dmps_path)
