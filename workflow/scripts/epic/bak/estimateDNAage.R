...
estimate.dnam.age <- function(data,goldstandard="probeAnnotation21kdatMethUsed.csv",dat.clock="AdditionalFile3.csv",trafo=function(x,adult.age=20){x <- (x+1)/(1+adult.age); ifelse(x<=1,log(x),x-1)},anti.trafo=function(x,adult.age=20){ifelse(x<0,(1+adult.age)*exp(x)-1,(1+adult.age)*x+adult.age)},fastImputation=FALSE,normalizeData=TRUE,alwaysImpute=FALSE,meanXchromosome=NULL) {
  ...
  if (normalizeData ){
    ...
    ol_probes <- intersect(colnames(datMethUsed), as.character(goldstandard$Name))
    datMethUsed_sub <- datMethUsed[,ol_probes]
    goldstandard_sub <- goldstandard[which(goldstandard$Name %in% ol_probes),]
    
    datMethUsedNormalized <-
      BMIQcalibration(datM=datMethUsed_sub,
                      goldstandard.beta=goldstandard_sub$goldstandard2,
                      plots=FALSE)
  } else { 
  ...
  if (nSamples>1 ) {
    ...
    datMethClock= data.frame(datMethClock0[colnames(datMethClock0) %in% as.character(dat.clock$CpGmarker[-1])])
    dim(datMethClock)
    predictedAge=as.numeric(anti.trafo(dat.clock$CoefficientTraining[1]+as.matrix(datMethClock)%*% as.numeric(dat.clock[dat.clock$CpGmarker %in% colnames(datMethClock),]$CoefficientTraining)))
  } # end of if
