...
samplesDir <- file.path("data","samples", "vedolizumab", "01_discovery")
commondataDir <- file.path("/home", "ayliyim", "archive", "common_data")

#setup output directories
outputDir <- file.path("output","01_DM_analyses", "vedolizumab", "201116")
...
#import samples
#samplesheet <- read.metharray.sheet(base = samplesDir, pattern = "samples_vedolizumab_PRJ0000008_CDMEDRESP_v6.csv")
samplesheet <- readxl::read_excel(file.path(samplesDir, "samples_vedolizumab_PRJ0000008_CDMEDRESP_v9.xlsx"))
samplesheet$Basename <- samplesheet$Filepath
samplesheet$resptime <- with(samplesheet, paste0(Response, "_", Timepoint))

#prepare rgset
rgset_vedo <- read.metharray.exp(targets = samplesheet)
...
if(!file.exists(file.path(edaDir, "estimatedCellProportion.csv"))){
  ...
  cellDist <- data.frame(cellDist, 
                         ...
                         timepoint = pData(rgset_vedo)$Timepoint,
                         donor = pData(rgset_vedo)$Biobank_ID,
...
mvals_vedo_svd_df <- data.frame(Sample_ID = pData(gmset_vedo_filtered)$Sample_Name,
                                Origin = pData(gmset_vedo_filtered)$Biobank_ID,
                                Timepoint = paste0("T", pData(gmset_vedo_filtered)$Timepoint),
                                Response = pData(gmset_vedo_filtered)$Response,
                                Sex = pData(gmset_vedo_filtered)$predictedSex,
                                Age = pData(gmset_vedo_filtered)$Age_baseline,
...
mvals_vedo_svd_plotobj <- ggplot(mvals_vedo_svd_df, aes(x = PC1, y = PC2, Label = Sample_ID, Origin = Origin, Timepoint = Timepoint, Response = Response, Sex = Sex, Age = Age)) +
  geom_point(aes(fill = Response, shape = Timepoint), size = 3) +
  theme_bw() +
...
Cairo(width = 1000, height = 1000, type = "pdf", file = file.path(edaDir, "PCA_timesplit.pdf"), bg = "white", units = "px", dpi = 120)
print(mvals_vedo_svd_plotobj + facet_wrap(~Timepoint)) +
dev.off()
```
...
RvNR_T3vT2vT1_top_all %>%
  ...
  labs(title = "Response-associated DMPs over time",
       subtitle = "Mean difference in methylation ± SE") +
  xlab("T2 vs T1") +
  ylab("T3 vs T2") +
