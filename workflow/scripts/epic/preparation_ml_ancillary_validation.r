...
t1_discancval_samples <- pData(rgset) %>%
  data.frame() %>%
  dplyr::filter(!!sym(timepoint_column) %in% c("T1"),
                !!sym(cohort_column) %in% c("EPIC-CD Discovery", "EPIC-CD Validation", "Ancillary validation"),
                !is.na(!!sym(response_column)),
                Disease == "CD",
                !(!!sym(cohort_column) == "EPIC-CD Validation" & Center_source == "John Radcliffe Hospital")) %>%
  tibble::rownames_to_column(var = "arrayname") %>%
  dplyr::pull(arrayname)

rgset_t1_discancval <- rgset[,colnames(rgset) %in% t1_discancval_samples]

# Normalization

## Functional normalization

gmset <- preprocessFunnorm(rgSet = rgset_t1_discancval)
