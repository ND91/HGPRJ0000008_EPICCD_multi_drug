...
dmps_RvNRvT2vT1_ada <- read.csv(dmps_ada_path) %>%
  dplyr::select(CGID, t_RvNRvT2vT1, P.Value_RvNRvT2vT1, adj.P.Val_RvNRvT2vT1, Betadiff_T1RvNR, Betadiff_T2RvNR) %>%
  dplyr::mutate(Treatment = "Adalimumab")
dmps_RvNRvT2vT1_ifx <- read.csv(dmps_ifx_path) %>%
  dplyr::select(CGID, t_RvNRvT2vT1, P.Value_RvNRvT2vT1, adj.P.Val_RvNRvT2vT1, Betadiff_T1RvNR, Betadiff_T2RvNR) %>%
  dplyr::mutate(Treatment = "Infliximab")
dmps_RvNRvT2vT1_vdz <- read.csv(dmps_vdz_path) %>%
  dplyr::select(CGID, t_RvNRvT2vT1, P.Value_RvNRvT2vT1, adj.P.Val_RvNRvT2vT1, Betadiff_T1RvNR, Betadiff_T2RvNR) %>%
  dplyr::mutate(Treatment = "Vedolizumab")
dmps_RvNRvT2vT1_ust <- read.csv(dmps_ust_path) %>%
  dplyr::select(CGID, t_RvNRvT2vT1, P.Value_RvNRvT2vT1, adj.P.Val_RvNRvT2vT1, Betadiff_T1RvNR, Betadiff_T2RvNR) %>%
  dplyr::mutate(Treatment = "Ustekinumab")

dmps_RvNRvT2vT1 <- rbind(dmps_RvNRvT2vT1_ada, dmps_RvNRvT2vT1_ifx, dmps_RvNRvT2vT1_vdz, dmps_RvNRvT2vT1_ust)

dmps_RvNRvT2vT1$Significant <- ifelse(dmps_RvNRvT2vT1$adj.P.Val_RvNRvT2vT1<0.05, "Significant", "NS")

# Volcanoplot

vplot_all <- dmps_RvNRvT2vT1 %>%
  ggplot(aes(x = Betadiff_T1RvNR, y = Betadiff_T2RvNR, col = Significant)) +
  geom_hline(yintercept = 0, col = "grey") +
  geom_vline(xintercept = 0, col = "grey") +
  geom_point_rast() +
  geom_point_rast(data = dmps_RvNRvT2vT1 %>% dplyr::filter(Significant == "Significant"), col = "black") +
  labs(title = "Responders vs non-responders over time",
       x = "%Methylation difference at T1",
       y = "%Methylation difference at T2") +
...
data.frame(Beta = getBeta(gmset_vdz)["cg21932983",],
           Response = pData(gmset_vdz)$Response,
           Timepoint = pData(gmset_vdz)$Timepoint,
           Donor = pData(gmset_vdz)$DonorID) %>%
  ggplot(aes(x = Timepoint, y = Beta, col = Response)) +
  geom_boxplot() +
  geom_jitter() +
  theme_bw()
